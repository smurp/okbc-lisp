...
(ok-utils:defdoc structure-trie (structure)
...
(ok-utils:defdoc make-structure-trie (function)
  "Makes a new empty <code>structure-trie</code> node.")

(ok-utils:defdoc structure-trie-p (function)
  "A predicate that is true of <code>structure-trie</code> nodes.")

(ok-utils:defdoc structure-trie-value (function)
  "Accessor to get to the value slot of a <code>structure-trie</code> node.")

(ok-utils:defdoc structure-trie-arcs (function)
...
(ok-utils:defdoc root-trie (structure)
...
;------------------------------------------------------------------------------
(ok-utils:with-substitution-groups
     ((("STRUCTURE-TRIE" "TRIE")))
...
(ok-utils:defdoc trie-remove-duplicate-state (structure)
...
(ok-utils:defdoc make-trie-remove-duplicate-state (function)
  "Constructor for <code>trie-remove-duplicate-state</code>s.")

(ok-utils:defdoc trie-remove-duplicate-state-count (function)
  "The number of objects held within the
   <code>trie-remove-duplicate-state</code>.")

(ok-utils:defdoc trie-remove-duplicate-state-unique (function)
...
(ok-utils:defokbcgeneric copy-deep-structure-trie (structure-trie)
...
(ok-utils:defokbcgeneric reconstitute-from-hash-key (x)
  (:documentation "The inverse operation of <code>ok-utils:fast-hash-key</code>.
...
(ok-utils:defokbcgeneric copy-deep-hybrid-trie (trie)
...
(ok-utils:defokbcgeneric delete-hybrid-trie (key trie)
  (:documentation "Given a hybrid <code>trie</code> or
...
(ok-utils:defokbcgeneric dismantle-hybrid-trie (trie)
  (:documentation "Given a hybrid <code>trie</code> or
...
(ok-utils:defokbcgeneric get-hybrid-trie (key trie)
...
(ok-utils:defokbcgeneric get-hybrid-trie-returning-node
...
(ok-utils:defokbcgeneric get-hybrid-trie-returning-node-no-extend
    (key trie &optional default)
  (:documentation "Returns the value for a <code>key</code> in a
   <code>trie</code>, and t/nil if found.  The trie may consist of some
   mixture of <code>trie</code> instances and <code>structure-trie</code>
...
(ok-utils:defokbcgeneric maphybrid-trie (f trie &optional parent)
...
(ok-utils:defokbcgeneric maphybrid-trie-nodes (f trie &optional with-values)
...
(ok-utils:defokbcgeneric maphybrid-trie-nodes-remove-if (f trie &optional key)
...
(ok-utils:defokbcgeneric set-hybrid-trie-value (trie value)
...
(ok-utils:defokbcgeneric hybrid-trie-arcs (trie)
...
(ok-utils:defokbcgeneric hybrid-trie-arcs-hashed-p (trie)
...
(ok-utils:defokbcgeneric hybrid-trie-value (trie)
...
(ok-utils:defokbcgeneric hybrid-trie-stash (trie)
